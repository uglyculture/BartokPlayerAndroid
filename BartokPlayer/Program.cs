...
const string StreamUrl = "http://icast.connectmedia.hu/4742/mr3hq.mp3";
...
while (!cts.Token.IsCancellationRequested)
{
    ...
    try
    {
        Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] Connecting...");

        using var client = new HttpClient();
        client.Timeout = Timeout.InfiniteTimeSpan;
        using var connectCts = CancellationTokenSource.CreateLinkedTokenSource(cts.Token);
        connectCts.CancelAfter(TimeSpan.FromSeconds(10));
        using var response = await client.GetAsync(StreamUrl, HttpCompletionOption.ResponseHeadersRead, connectCts.Token);
        response.EnsureSuccessStatusCode();
        connectCts.CancelAfter(Timeout.InfiniteTimeSpan); // disable connect timeout once connected
